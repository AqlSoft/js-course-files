        ...
        <div class="col col-12">
            <fieldset class="mt-4 mx-0 mb-0">
                ...
                <div class="input-group">
                    <button class="py-0 pt-1 pb-1 ms-4 btn btn-primary active"
                        data-type="{{ $direction_input }}">Input</button>
                    <button class="py-0 pt-1 pb-1 btn btn-outline-secondary"
                        data-type="{{ $direction_output }}">Output</button>
                </div>
                                ...
                                @foreach ($receipts as $receipt)
                                    ...
                                    @if ($receipt->direction === $direction_input)
                                        <tr>
                                            ...
                                            @if ($receipt->status === 1)
                                                <td><span class="badge bg-info">{{ $status[$receipt->status] }}</span>
                                                </td>
                                                <td>
                                                    <a class="btn btn-sm py-0 p-0" data-bs-toggle="tooltip"
                                                        title="insert Receipt"
                                                        href="{{ route('add-store-input-entry', [$receipt->id]) }}">
                                                        <i class="fa fa-square-plus text-success"></i>
                                                    </a>
                                                    <a class="btn btn-sm py-0 p-0" data-bs-toggle="tooltip"
                                                        title="print Receipt" href="">
                                                        <i class="fa fa-print text-dark"></i>
                                                    </a>
                                                </td>
                                            @elseif($receipt->status === 0)
                                                <td><span class="badge bg-warning">{{ $status[$receipt->status] }}</span>
                                                </td>
                                                <td>
                                                    <a class="btn btn-sm py-0 p-0" data-bs-toggle="tooltip"
                                                        title="edit Receipt"
                                                        href="{{ route('edit-receipt-info', $receipt->id) }}">
                                                        <i class="fa fa-edit text-primary"></i>
                                                    </a>
                                                    <a class="btn btn-sm py-0 p-0" data-bs-toggle="tooltip"
                                                        title="delete Receipt"
                                                        onclick="if (!confirm('You are going to delete this receipt, are you sure?'))return false"
                                                        href="{{ route('destroy-receipt-info', $receipt->id) }}">
                            ...
                            @else
                                <tr>
                                    <td colspan="7">No Receipts has been added yet, Add your <a class=""
                                            data-bs-toggle="collapse" data-bs-target="#addreceiptForm">first Receipt</a>.
                                    </td>
                                </tr>
                            @endif
                        </tbody>
